...
void parse(List<Map> description) {
    description.each {
        if (it.name in ["current", "apparentPower", "reactivePower", "powerFactor", "energyToday",
            ...
            if(it.name == "energyToday") {
                newEnergy = it.value
                String pattern = /(\d*[0-9]\d*(\.\d+)?|0*\.\d*[0-9]\d*)/
                java.util.regex.Matcher matching = newEnergy =~ pattern
                newEnergy = matching[0][1].toDouble()
                sendEvent(name: "energy", value: newEnergy)
            }
            sendEvent(it)
        } else {
